...
public class DemoCameraDialogActivity extends AppCompatActivity implements CameraDialg2.FileSavedEventListener {
    ...
    public void takePic(View view) {
        ...
        //添加权限检查 适配特殊定制机型
        CameraDialg2 cameraDialg2 = CameraDialg2.newInstance();
        if (cameraDialg2 != null) {
            //可以使用匿名内部类的样式进行结果回调，也可以是 容器实行相关接口；最好的使用EventBus进行解耦
//        cameradialg.setFileSavedEventListener(new CameraDialg2.FileSavedEventListener() {
//            @Override
...
//            }
//        });
            cameraDialg2.showSafe(getSupportFragmentManager(), "camera");
        }
